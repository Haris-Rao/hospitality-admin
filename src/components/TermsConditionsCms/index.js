...
import classes from "./TermsConditionsCms.module.css";
...
export default function TermsConditionsCms() {
  ...
  const handleSubmit = async () => {
    ...
    const url = BaseURL("cms/terms_conditions");
    const response = await Patch(url, params, apiHeader(access_token));
    if (response !== undefined) {
      RenderToast({
        type: "success",
        message: "Terms & Conditions CMS updated successfully",
      });
    }
    setLoading(false);
  };
  ...
  const getData = async () => {
    ...
    if (response !== undefined) {
      const res = response?.data?.data?.terms_conditions;
